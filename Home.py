...
with params_col:
    ...
    with st.form(key = 'params_form'):
        ...
        listado_riesgos = ['Terremotos', 'Tsunamis', 'Volcanes', 'Tornados', 'Huracanes', 'Inundaciones', 'Sequías y Calor Extremo', 'Incendios', 'Nevadas', 'Avalanchas']
        modo = st.selectbox('Selecciona un tipo de riesgo climático a monitorizar', listado_modos, key = 'modo')

        listado_edad_retiro = ['50 años','55 años','60 años','65 años','70 años']
        retiro = st.selectbox('¿A cuántos años deseas financiar tus seguros?', listado_edad_retiro, key = 'retiro')
        
        estilo_de_vida_retiro = st.selectbox('Tipos de Industrias', ['Energética', 'Hidrocarburos', 'Minera', 'Agropecuaria'], key = 'interval_selectbox')
        ahorro = st.number_input('Monto a asegurar en COP millones', min_value = 10, max_value = 500, value = 365, step = 1, key = 'period_no_input')
        a = st.slider("Monto a pagar prima mensual:", 0, 10000, 500)
        st.divider()
